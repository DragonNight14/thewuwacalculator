/*
  Author: Runor Ewhro
  Description: builds shared stat displays and nested stat trees for
               calculator surfaces that need formatted labels, icons, colors,
...
export interface StatsView {
  mainStats: StatViewRow[]
  secondaryStats: StatViewRow[]
...
export const STAT_ICON_MAP: Record<string, string> = {
  ...
  'Energy Regen': '/assets/game/stats/icons/energyregen.png',
  'Crit Rate': '/assets/game/stats/icons/critrate.png',
  'Crit DMG': '/assets/game/stats/icons/critdmg.png',
  'Healing Bonus': '/assets/game/stats/icons/healing.png',
  'Tune Break Boost': '/assets/game/stats/icons/tune-break-boost.png',
  'Basic Attack DMG Bonus': '/assets/game/stats/icons/basic.png',
  'Heavy Attack DMG Bonus': '/assets/game/stats/icons/heavy.png',
  'Resonance Skill DMG Bonus': '/assets/game/stats/icons/skill.png',
  'Resonance Liberation DMG Bonus': '/assets/game/stats/icons/liberation.png',
  'Aero DMG Bonus': '/assets/game/stats/icons/aero.png',
  'Glacio DMG Bonus': '/assets/game/stats/icons/glacio.png',
  'Spectro DMG Bonus': '/assets/game/stats/icons/spectro.png',
  'Fusion DMG Bonus': '/assets/game/stats/icons/fusion.png',
  'Electro DMG Bonus': '/assets/game/stats/icons/electro.png',
  'Havoc DMG Bonus': '/assets/game/stats/icons/havoc.png',
}

// compact number formatter for large damage/stat readouts
export function formatCompactNum(raw: number | null): string {
...
// format stat values according to whether they are flat or percent-based
export function formatStatValue(label: string, value: number): string {
...
export function makeStatsView(runtime: ResRuntime, finalStats: FinalStats): StatsView {
...
export interface StatTreeBranch {
  kind: 'branch'
  key: string
  label: string
  color?: string
...
export type StatTreeNode = StatTreeLeaf | StatTreeBranch
...
function makeModChildren(buff: ModBuff): StatTreeLeaf[] {
  const leaves: StatTreeLeaf[] = []
...
function makeBaseStat(key: string, label: string, stat: { base: number; final: number }): StatTreeLeaf {
...
export function makeStatsTree(finalStats: FinalStats): StatTreeNode[] {
  const root: StatTreeNode[] = []
  ...
  const ttrbKids: StatTreeNode[] = []
  for (const key of ATTR_KEYS) {
    const mods = makeModChildren(finalStats.attribute[key])
  ...
  const skllTypeKids: StatTreeNode[] = []
  for (const key of SKLLTYPEKEYS) {
    const mods = makeModChildren(finalStats.skillType[key])
    if (mods.length === 0) continue
    skllTypeKids.push({
      kind: 'branch',
      key,
