/*
  Author: Runor Ewhro
  Description: Serializes and parses benchmark resonator profile clipboard
               payloads so the benchmark rail can copy, cut, and paste full
...
export const PROF_CLIP_KEY = 'benchmark-profile-clipboard'
...
export function parseProfClip(raw: string): ProfileClipPay | null {
  ...
  try {
    ...
    if (
      (parsed.kind !== PROF_CLIP_KEY && parsed.kind !== LEGACY_PROF_CLIP_KEY)
