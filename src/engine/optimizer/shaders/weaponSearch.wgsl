/*
  Author: Runor Ewhro
  Description: evaluates theory optimizer echo combos against compact weapon
               overlays so the gpu can keep the best weapon and echo pairing in
               one candidate record.
*/

// this kernel is the weapon-search variant of target.wgsl: it evaluates each
// theory combo against every weapon overlay (a compact per-weapon stat delta
// composed onto the shared base context) and keeps the best, packing the chosen
// weapon index into the candidate. only the batch (theory) path is supported.
...
// per-weapon stat overlays. each weapon's 16 overlay floats (WEAPON_OVERLAY_SLOTS
// order) are stored as 4 consecutive vec4s, so weapon w occupies vec4s
// [w*4, w*4+4). a uniform (not storage) buffer to stay under the 10 storage-
// buffer per-stage limit; the data is tiny (<=31 weapons * 64 bytes). sized for
// up to 32 weapons (32 * 4 = 128 vec4s).
@group(0) @binding(13) var<uniform> weaponOverlays: array<vec4<f32>, 128>;
...
@group(0) @binding(12) var<uniform> weaponMeta: WeaponMeta;

// theory batch pins the main echo at slot 0, freeing the main-position bits;
// the chosen weapon index goes in the high bits instead (must match
// WEAPON_INDEX_SHIFT in constants.ts).
...
fn jingranFireOfLifeMultiplier(charId: f32, skillId: u32, finalHp: f32, toggles: f32) -> f32 {
    ...
    let seqBoost = select(1.0, 1.4, decodeSequence(p) >= 2.0);
...
struct ComboEval {
    ...
    // which weapon overlay produced the best damage
    weapon: u32,
};

// compose a per-weapon Params from the shared base context plus weapon w's
// overlay. only the weapon-varying fields are overridden; everything else
// (scaling, enemy multipliers, skill id/meta, archetype) stays from base.
// overlay field order must match WEAPON_OVERLAY_SLOTS in config/constants.ts.
fn applyOverlay(w: u32) -> Params {
    var p = params;
    let base4 = w * 4u;
    // 4 vec4s == the 16 overlay floats in WEAPON_OVERLAY_SLOTS order
    let v0 = weaponOverlays[base4 + 0u];
    let v1 = weaponOverlays[base4 + 1u];
    let v2 = weaponOverlays[base4 + 2u];
    let v3 = weaponOverlays[base4 + 3u];
    p.baseAtk           = v0.x;
    p.finalAtk          = v0.y;
    p.finalHp           = v0.z;
    p.finalDef          = v0.w;
...
fn evalMainPos(
    ...
    if (pre.charId == 1212.0) {
        var extraAtk = min(max(pre.finalHpBase * 0.036, 0.0), 1800.0);
        if (decodeSequence(p) >= 3.0 && jingranEverflow(pre.toggles)) {
            extraAtk = min(max(pre.finalHpBase * 0.05, 0.0), 2500.0);
        }
        finalAtk += extraAtk;
    }
...
// evaluate one exact 5-echo combo against every weapon overlay and keep the
// best. theory batch pins the main echo at slot 0, so the only loop is over
// weapons. the echo aggregation (buildEchoBase/applySetEffects) is shared across
// weapons; only buildPreMain + evalMainPos run per weapon.
fn computeDamageForEchoIds(echoIds: array<i32, 5>) -> ComboEval {
    ...
    // shared across weapons (skill mask / set mask are weapon-invariant)
    let sonata = applySetEffects(base, skillMask, setRuntimeMask);

    // theory batch: main echo is pinned at slot 0
    let mainId = echoIds[0];
    if (mainId < 0) { return ComboEval(0.0, 0u, 0u); }
    if (lockedIndex >= 0 && mainId != lockedIndex) { return ComboEval(0.0, 0u, 0u); }
    ...
    let wcount = weaponMeta.weaponCount;

    for (var w: u32 = 0u; w < wcount; w = w + 1u) {
        let p = applyOverlay(w);
    ...
    return ComboEval(best, 0u, bestWeapon);
}
...
fn main(
    ...
    if (batchMode) {
        if (best > 0.0) {
            // theory batch pins the main at slot 0, so the high bits carry the
            // chosen weapon index instead of the main position.
            let packedIdx = (bestWeapon << WEAPON_INDEX_SHIFT_C) | bestIndex;
